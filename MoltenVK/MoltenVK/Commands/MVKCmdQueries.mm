...
VkResult MVKCmdBeginQueryIndexed::setContent(MVKCommandBuffer *cmdBuff,
                                             VkQueryPool queryPool, uint32_t query, uint32_t flags,
                                             uint32_t index) {
    _mtlIndexedQueryPool = (MVKQueryPool*)queryPool;
    _query = query;
    _mtlQueryIndexedFlags = flags;
    _mtlQueryIndexedIndex = index;

    VkResult rslt = MVKCmdQuery::setContent(cmdBuff, queryPool, query);
    _mtlIndexedQueryPool->beginQueryAddedTo(_query, cmdBuff);
...
void MVKCmdBeginQueryIndexed::encode(MVKCommandEncoder *cmdEncoder) {
    cmdEncoder->_isIndexedDraw = true;
    //This should only be called in transform feedback mode and multiview is disallowed in that mode so just call
    // beginQuery
    _mtlIndexedQueryPool->beginQuery(_query, _mtlQueryIndexedFlags, cmdEncoder);
}

#pragma mark -
#pragma mark MVKCmdEndQueryIndexed

VkResult MVKCmdEndQueryIndexed::setContent(MVKCommandBuffer *cmdBuffer,
                                           VkQueryPool _queryPool, uint32_t _query, uint32_t _index) {
    MVKCmdQuery::setContent(cmdBuffer, _queryPool, _query);
    index = _index;
}

void MVKCmdEndQueryIndexed::encode(MVKCommandEncoder *cmdEncoder) {
    uint32_t query = _query;
    if (cmdEncoder->getMultiviewPassIndex() > 0)
        query += cmdEncoder->getSubpass()->getViewCountUpToMetalPass(cmdEncoder->getMultiviewPassIndex() - 1);
    _queryPool->endQuery(query, cmdEncoder);
