                               ...
                               const MVKResourceBinder& RESTRICT binder)
{
	...
	executeBindOps(encoder, mvkEncoder, common, implicitBufferData, resources.bindScript.ops, useResourceStage, exists, bindings, binder);
	...
	const VkRect2D* scissors)
{
	...
	if (flags.hasAny(FlagsViewportScissor)) {
		_stateReady.addAll(flags & FlagsViewportScissor);
	...
	if (flags.hasAny(FlagsMetalState)) {
		_stateReady.addAll(flags & FlagsMetalState);
...
#define PICK_STATE(x) (dynamicStateFlags.has(MVKRenderStateFlag::x) ? &dynamicStateData : &staticStateData)
	...
	if (anyStateNeeded.hasAny(FlagsWithEnable)) {
		_stateReady.addAll(FlagsWithEnable);
...
#undef PICK_STATE
	...
	MVKRenderStateFlags staticStateNeeded = staticStateFlags.removingAll(_stateReady);
	MVKRenderStateFlags dynamicStateNeeded = dynamicStateFlags.removingAll(_stateReady);
	if (!staticStateNeeded.empty()) {
		bindStateData(encoder, mvkEncoder, staticStateData, staticStateNeeded, pipeline->getViewports(), pipeline->getScissors());
	}
	if (!dynamicStateNeeded.empty()) {
		bindStateData(encoder, mvkEncoder, dynamicStateData, dynamicStateNeeded, vk._viewports, vk._scissors);
	}
...
void MVKOcclusionQueryCommandEncoderState::prepareHelperDraw(id<MTLRenderCommandEncoder> encoder, MVKCommandEncoder* mvkEncoder) {
	if (_mtlVisibilityResultMode != MTLVisibilityResultModeDisabled) {
		if (!_dirty) {
			...
			OcclusionQueryLocation& current = _mtlRenderPassQueries.back();
			beginOcclusionQuery(mvkEncoder, current.queryPool, current.query, _mtlVisibilityResultMode);
		}
	} else if (_dirty) {
		[encoder setVisibilityResultMode:MTLVisibilityResultModeDisabled
		                          offset:mvkEncoder->_pEncodingContext->mtlVisibilityResultOffset];
