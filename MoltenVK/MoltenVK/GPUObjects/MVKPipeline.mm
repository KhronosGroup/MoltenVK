...
void MVKGraphicsPipeline::encode(MVKCommandEncoder* cmdEncoder, uint32_t stage) {
    ...
    cmdEncoder->_graphicsResourcesState.bindSwizzleBuffer(
			_swizzleBufferIndex,
			_needsVertexSwizzleBuffer,
			_needsTessCtlSwizzleBuffer,
			_needsTessEvalSwizzleBuffer,
#if MVK_XCODE_14
			_needsTaskSwizzleBuffer,
			_needsMeshSwizzleBuffer,
#endif
...
#if MVK_XCODE_14
			_needsTaskBufferSizeBuffer,
			_needsMeshBufferSizeBuffer,
#endif
			_needsFragmentBufferSizeBuffer);
	cmdEncoder->_graphicsResourcesState.bindDynamicOffsetBuffer(
...
#if MVK_XCODE_14
			_needsTaskDynamicOffsetBuffer,
			_needsMeshDynamicOffsetBuffer,
#endif
			_needsFragmentDynamicOffsetBuffer);
    cmdEncoder->_graphicsResourcesState.bindViewRangeBuffer(
			_viewRangeBufferIndex,
			_needsVertexViewRangeBuffer,
#if MVK_XCODE_14
...
template<typename T>
id<MTLRenderPipelineState> MVKGraphicsPipeline::getOrCompilePipeline(T* plDesc,
...
void MVKGraphicsPipeline::initMTLRenderPipelineState(const VkGraphicsPipelineCreateInfo* pCreateInfo,
	...
	if (isMeshPipeline()) {
#if MVK_XCODE_14
		MTLMeshRenderPipelineDescriptor* plDesc = newMTLMeshRenderPipelineDescriptor(pCreateInfo, reflectData, pTaskSS, pTaskFB, pMeshSS, pMeshFB, pFragmentSS, pFragmentFB);	// temp retain
		if(plDesc) {
			const VkPipelineRenderingCreateInfo* pRendInfo = getRenderingCreateInfo(pCreateInfo);
...
MTLMeshRenderPipelineDescriptor* MVKGraphicsPipeline::newMTLMeshRenderPipelineDescriptor(const VkGraphicsPipelineCreateInfo* pCreateInfo,
	...
	std::string errorLog;
	if(pTaskSS) {
		if (!getShaderOutputs(((MVKShaderModule*)pTaskSS->module)->getSPIRV(), spv::ExecutionModelTaskEXT, pTaskSS->pName, taskOutputs, errorLog) ) {
			setConfigurationResult(reportError(VK_ERROR_INITIALIZATION_FAILED, "Failed to get task outputs: %s", errorLog.c_str()));
			return nil;
		}

		if(!addTaskShaderToPipeline(plDesc, pCreateInfo, shaderConfig, pTaskSS, pTaskFB)) { return nil; }
	} else {
		_mtlObjectThreadgroupSize = MTLSizeMake(1, 1, 1);
	}

	SPIRVShaderOutputs meshOutputs;
	if (!getShaderOutputs(((MVKShaderModule*)pMeshSS->module)->getSPIRV(), spv::ExecutionModelMeshEXT, pMeshSS->pName, meshOutputs, errorLog) ) {
...
bool MVKGraphicsPipeline::addTaskShaderToPipeline(MTLMeshRenderPipelineDescriptor* plDesc,
	...
	if (pTaskSS) {
        ...
        MVKMTLFunction func = getMTLFunction(shaderConfig, pTaskSS, pTaskFB, "Task");
        id<MTLFunction> mtlFunc = func.getMTLFunction();
        if ( !mtlFunc ) {
...
bool MVKGraphicsPipeline::addMeshShaderToPipeline(MTLMeshRenderPipelineDescriptor* plDesc,
	...
	if (pMeshSS) {
        ...
        shaderConfig.options.mslOptions.swizzle_buffer_index = _swizzleBufferIndex.stages[kMVKShaderStageMesh];
        shaderConfig.options.mslOptions.indirect_params_buffer_index = _indirectParamsIndex.stages[kMVKShaderStageMesh];
        shaderConfig.options.mslOptions.shader_output_buffer_index = _outputBufferIndex.stages[kMVKShaderStageMesh];
        shaderConfig.options.mslOptions.buffer_size_buffer_index = _bufferSizeBufferIndex.stages[kMVKShaderStageMesh];
        shaderConfig.options.mslOptions.dynamic_offsets_buffer_index = _dynamicOffsetBufferIndex.stages[kMVKShaderStageMesh];
        shaderConfig.options.mslOptions.view_mask_buffer_index = _viewRangeBufferIndex.stages[kMVKShaderStageMesh];
        shaderConfig.options.mslOptions.capture_output_to_buffer = false;
        shaderConfig.options.mslOptions.disable_rasterization = !_isRasterizing;

        MVKMTLFunction func = getMTLFunction(shaderConfig, pMeshSS, pMeshFB, "Mesh");
        id<MTLFunction> mtlFunc = func.getMTLFunction();
        if ( !mtlFunc ) {
...
template<typename T>
id<MTLRenderPipelineState> MVKRenderPipelineCompiler::newMTLRenderPipelineState(T* mtlRPLDesc) {
...
#if MVK_XCODE_14
			else if constexpr(std::is_same_v<T, MTLMeshRenderPipelineDescriptor>) {
