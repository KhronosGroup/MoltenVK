...
MTLCompileOptions* MVKDevice::getMTLCompileOptions(uint32_t fpFastMathMode,
	...
	switch (getMVKConfig().fastMathEnabled) {
		case MVK_CONFIG_FAST_MATH_ALWAYS:
			fpFastMathMode = mvk::kSPIRVFPFastMathModesSupported;
			break;
		case MVK_CONFIG_FAST_MATH_NEVER:
			fpFastMathMode = spv::FPFastMathModeMaskNone;
			break;
		default:
			break;
	}
...
#if MVK_XCODE_16
	// If VK_KHR_shader_float_controls2 is enabled, honour the float control settings, which includes
	// always using precise MSL function calls, to provide behaviour most consistent with Vulkan.
	// If VK_KHR_shader_float_controls2 is disabled, we can be more aggressive,
	// and use fast MSL function calls if allowed by Fast Math config settings.
	// TODO: Some MSL functions have different behavour than Vulkan and should probably been overwritten
	// in shader compilation. For example, MSL fract() doesn't handle Inf->NaN like Vulkan.
	if (_enabledShaderFloatControls2Features.shaderFloatControls2 &&
		[mtlCompOpt respondsToSelector: @selector(mathMode)]) {
		MTLMathMode mtlMathMode = MTLMathModeSafe;
		MTLMathFloatingPointFunctions mtlFPFuncs = MTLMathFloatingPointFunctionsPrecise;
		if (mvkAreAllFlagsEnabled(fpFastMathMode, (spv::FPFastMathModeNSZMask | spv::FPFastMathModeAllowRecipMask |
													  spv::FPFastMathModeAllowReassocMask | spv::FPFastMathModeAllowContractMask))) {
			mtlMathMode = MTLMathModeRelaxed;
			if (mvkAreAllFlagsEnabled(fpFastMathMode, (spv::FPFastMathModeNotNaNMask | spv::FPFastMathModeNotInfMask))) {
...
#endif
	{
		mtlCompOpt.fastMathEnabled = mvkAreAllFlagsEnabled(fpFastMathMode, mvk::kSPIRVFPFastMathModesSupported);
	}
