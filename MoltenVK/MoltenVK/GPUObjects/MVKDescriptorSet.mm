...
#undef DISPATCH
		...
		while (true) {
			switch (binding->gpuLayout) {
#define DISPATCH(x) writeDescriptorSetGPUBuffer<ArgBufMode, MVKDescriptorGPULayout::x>(*binding, enc, base, auxOffsets, src, srcStride, srcType, start, std::min(count, binding->descriptorCount - start))
...
#undef DISPATCH
			...
			if (start + count <= binding->descriptorCount)
				break;
			count -= binding->descriptorCount - start;
			start = 0;
